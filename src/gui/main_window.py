...
class MainWindow:
    ...
    def start_message_monitor(self):
        ...
        self.root.after(10000, self.start_message_monitor)

    def check_new_messages(self):
        """Проверка новых сообщений в логах"""
        if hasattr(self, 'logger'):
            current_count = len(self.logger.get_logs())
            if current_count > self.last_message_count and self.last_message_count > 0:
                new_count = current_count - self.last_message_count
                if new_count > 0:
                    msg = f"Получено {new_count} новое сообщение" if new_count == 1 else f"Получено {new_count} новых сообщений"
                    self.show_toast("📬 Новые сообщения", msg, duration=3)
    ...
    def show_notification(self, message, type="info"):
        ...
        self.show_toast(title, message, duration=3)
    ...
    def on_send_success(self):
        ...
        self.show_toast("📨 Сообщение отправлено", f"Кому: {self.send_recipient.get()}", duration=3)
    ...
    def start_service(self):
        try:
            ...
            self.show_toast("✅ Сервис запущен", "Jabber Robot начал работу", duration=3)
            self.save_state_for_recovery()
        except Exception as e:
            self.show_notification(f"Ошибка: {e}", "warning")
            self.show_toast("❌ Ошибка", f"Не удалось запустить сервис: {e}", duration=5)
    ...
    def stop_service(self):
        ...
        self.show_toast("⏹ Сервис остановлен", "Jabber Robot завершил работу", duration=3)
        self.save_state_for_recovery()
